...
settings = {
    ...
    'task'               : 'frame_classification', # experiment task (options: 'segmentation', 'frame_classification', 'video_classification')
    'aux_classification' : False,       # add auxiliary loss (only applies when 'task' equals segmentation)
    # data   
    'label_folder'       : 'frames_1',  # label folder name ('frames_1_disk_4mm' for segmentation, 'frames_1' for frame classification, and 'frames_16' for video classification)
    ...
    'epochs'             : 10,         # maximum number of epochs in training, 100 epochs takes too long
    ...
    'model'              : 'ViT', # network name, change to test a specific model
    'pretrained'         : True,       # determines whether pretrained or randomly initialized weights are used
    'input_channels'     : 3,           # number of input channels (generally 3 for pretrained because of rgb and 1 for randomly initialized is assumed)
    ...
    'focal_class_weights': [1,1],     # class weighting for foreground and background (only applies to focal loss or combinations including focal loss)
    'loss_weights'       : [1,1],      # loss weighting (only applies to combinations of two losses)
    'aux_weight'         : 0,           # auxiliary loss weighting (only applies if 'aux_classification' equals True)
    # optimization settings
    'optimizer'          : 'adam',      # optimizer name
    'learning_rate'      : 0.00000002,       # learning rate value
