...
def process_clip(
    ...
    for i, frame in enumerate(reader):
        ...
        processed_frame, processing = crop_and_resize(
            frame, points, output_shape=output_shape, apply_mask=apply_mask
        )
        # save the processed image and add the processing settings to the list
        imwrite(
            os.path.join(
                output_dir,
                os.path.basename(clip).replace(
                    ".mp4", f"_{str(i).zfill(3)}{output_type}"
                ),
            ),
            processed_frame,
        )
...
if __name__ == "__main__":
    ...
    cases = sorted(
        [item for item in os.listdir(input_folder) if item.lower().startswith("case")]
    )
    ...
    for case in tqdm(cases):
        ...
        points = [
            points_dict[case][os.path.splitext(os.path.split(clip)[1].split("_")[2])[0]]
            for clip in clips
        ]

        # set some inputs of the convert_clip function
        process = lambda clips, points: process_clip(
            clips, points, case_output, output_datatype, output_shape
        )
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for i, result in enumerate(results):
                processing_clips[
                    os.path.splitext(os.path.split(clips[i])[1].split("_")[2])[0]
                ] = result
